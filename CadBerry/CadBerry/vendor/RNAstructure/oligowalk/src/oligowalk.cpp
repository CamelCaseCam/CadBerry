...
int openseq_frac (structure *ct, RNA *rna, int start, int end) {
...
----------------------------------------------------------------------------------------------------*/
	...
	if (length==-1)
		...
		parser->setErrorSpecialized("Please enter the length of hybridiztion (e.g. -l 19 )");
	...
	RNA rna(seqfilename.c_str(), FILE_SEQ, true); // always open as RNA. (DNA data table is passed separately)
	...
	if (!openseq_frac(ct, &rna, start, end)) {
