	...
	for (i=start; i<=stop; ++i) {
		...
		//mode 2: refold  rna
			...
			if (foldsize ==0) {
				...
				//mode 2 +notrefold+ consider only the first suboptimal structure
				if (suboptimal ==0) {
					SAFE_DYNAMIC(ct,&data,1000,10,0,0,true);
				
					table[i][2] = ct->GetEnergy(1) - energy;
				}
				//mode 2 +notrefold+ consider all suboptimal structures with ensemble energy
				else if (suboptimal==1) {
				...
				//mode 2 +notrefold+ consider heuristic suboptimal structures with average energy
				else if (suboptimal==3) {
					sum = 0;
					sn = 0;
					for (k=1;k<=numofstructures;k++) {
				...
				//mode 2 +notrefold+ partionfunction
				else if (suboptimal ==2) {
					//save the energies in a save file
		...
		if (suboptimal==2) {	//-2 is unavailabe, as partition function is not availabe for inter oligo	
