...
public class DuplexFoldWindow
	 ...
	 */
	public DuplexFoldWindow( String acid ) {
		super( acid, acid + " DuplexFold" );
	}
	...
	protected void processCommand(final CommandInfo ci) {
		...
		switch (ci.command) {
			...
			case "Sequence File 2": {
				...
				// and return.
				String result = backend.buildDuplexFoldDataStructure(seq1, seq2, isRNA);
	...
	protected boolean runMainCalculation() {
		...
		showProgress();
		String result = backend.runDuplexFold( ct, percent, structures, window, save);
