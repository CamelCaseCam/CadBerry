...
public class Matrix3D implements java.io.Serializable, Cloneable {
    static final long serialVersionUID = 20160913L;
    ...
    private static final double EPSILON = 1.110223024E-16;
     ...
     * @return Returns true if inversion was successful, or false if the matrix is singular (non-invertible)
     *
     */
    public final boolean invert(Matrix3D m1) {
        return invertGeneral(m1);
    }
    ...
    private boolean invertGeneral(Matrix3D m1) {
        ...
        if (!luDecomposition(tmp, row_perm)) return false; //            throw new NoninvertibleTransformException("Matrix is singular.");
    ...
    public boolean equals(Object t1) {
        ...
        } catch (ClassCastException e1) { return false; } catch (NullPointerException e2) { return false; }
    }
    ...
    public int hashCode() {
        int bits = 1;
        bits ^= Double.hashCode(m01) << 2;
        bits ^= Double.hashCode(m01) << 3;
        bits ^= Double.hashCode(m02) << 5;
        bits ^= Double.hashCode(m10) << 7;
        bits ^= Double.hashCode(m11) << 11;
        bits ^= Double.hashCode(m12) << 13;
        bits ^= Double.hashCode(m20) << 17;
        bits ^= Double.hashCode(m21) << 19;
        bits ^= Double.hashCode(m22) << 23;
        return bits;
    }
     ...
     * @param xyz   the array that holds the values to be multiplied by this matrix and then replaced
     */
     ...
     * @param xyz   the array that holds the values to be multiplied by this matrix and then replaced
     */
    public final void transform(double[] xyz, int srcOffset, int dstOffset) {
        double x= xyz[srcOffset],y = xyz[srcOffset+1],z = xyz[srcOffset+2];
        xyz[dstOffset] = m00* x + m01*y + m02*z;
        xyz[dstOffset+1] = m10* x + m11*y + m12*z;
        xyz[dstOffset+2] = m20* x + m21*y + m22*z;
    }
