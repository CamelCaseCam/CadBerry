...
{
    CDB_BuildInfo(FileName);
    if (FileName[0] == '¬')
        FileName = CDB::Application::Get().OpenProject->Name;    //We can just get rid of the AutoGeneratedEntryPoint stuff
    std::stringstream ss;
    ...
    for (auto Region : Output.first)
    {
        if (Region.End != 0)
        {
            ss << "     CDS             " << (Region.Start + 1 < Region.End ? Region.Start + 1 : Region.End)  << ".." << Region.End << "\n                     /label=\"" << Region.Name << "\"\n";
        }
        else
        {
			//To mitigate some of the mangled regions generated by GIL
            ss << "     CDS             " << "1..1\n                     /label=\"" << Region.Name << "\"\n";
        }
    }
    ss << "ORIGIN\n";
